...
export const serialRulesApi={
 ...
 stocks:async(request:DropdownPageRequest,branchCode:string)=>unwrap(await api.post<Envelope<{items:Array<{id:number;erpStockCode:string;stockName:string;groupCode?:string|null}>;pageNumber:number;pageSize:number;totalCount:number;totalPages:number;hasNextPage:boolean}>>('/api/erp-mirror/stocks/paged',buildDropdownPagedBody(request,{sortBy:'erpStockCode',filters:[{column:'branchCode',operator:'equals',value:branchCode}]}),{signal:request.signal}))
};
