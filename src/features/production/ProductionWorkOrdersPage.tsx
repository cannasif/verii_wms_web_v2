...
function WorkOrderDrawer({value,close,createPlan,createTransfer,canCreatePlan,canCreateTransfer}:{value:PreparedNetsisProductionWorkOrder;close:()=>void;createPlan:()=>void;createTransfer:()=>void;canCreatePlan:boolean;canCreateTransfer:boolean}){
  ...
  return <div className="fixed inset-0 z-[80] flex justify-end bg-black/55" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)close();}}>
    <aside role="dialog" aria-modal="true" aria-label={`${value.workOrderNumber} reçete detayı`} className="h-full w-full max-w-3xl overflow-y-auto border-l border-[var(--wms-app-border)] bg-[var(--wms-app-bg)] shadow-2xl">
      <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-[var(--wms-app-border)] bg-[var(--wms-app-bg)] p-5"><div><p className="text-xs font-bold uppercase tracking-widest text-[var(--wms-brand-primary)]">Netsis iş emri / reçete</p><h2 className="mt-1 font-mono text-2xl font-black">{value.workOrderNumber}</h2><p className="mt-1 text-sm text-[var(--wms-app-text-muted)]">{value.productCode} · {value.productName}</p></div><button type="button" onClick={close} className="rounded-lg border border-[var(--wms-app-border)] p-2"><X className="size-5"/></button></header>
