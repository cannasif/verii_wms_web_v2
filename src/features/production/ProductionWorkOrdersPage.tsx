...
export function ProductionWorkOrdersPage(): ReactElement {
  ...
  const open = async (row: ProductionSourceWorkOrder) => {
    const rowKey=`${row.sourceType}:${row.sourceSystemCode}:${row.workOrderNumber}`;
    setDetailLoading(rowKey);
  ...
  return <section className="space-y-5">
    <header className="rounded-2xl border border-[var(--wms-app-border)] bg-[image:var(--wms-brand-gradient-soft)] p-6">
      <p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--wms-brand-primary)]">Üretim / {sourceLabel} kaynağı</p>
    ...
    <section className="rounded-2xl border border-[var(--wms-app-border)] bg-[var(--wms-app-panel)] p-4">
      ...
      <div className="max-h-[calc(100dvh-22rem)] overflow-auto rounded-xl border border-[var(--wms-app-border)]">
        <table className="w-full min-w-[1050px] text-left text-sm">
          ...
          <tbody>{rows.map(row=>{const rowKey=`${row.sourceType}:${row.sourceSystemCode}:${row.workOrderNumber}`;return <tr key={rowKey} onClick={()=>void open(row)} className="cursor-pointer border-t border-[var(--wms-app-border)] transition hover:bg-[var(--wms-brand-soft)]">
            ...
            <td className="p-3 text-right">{detailLoading===rowKey?<Loader2 className="ml-auto size-4 animate-spin"/>:<span className="font-bold text-[var(--wms-brand-primary)]">Reçeteyi aç →</span>}</td>
          </tr>})}</tbody>
        </table>
  ...
  return createPortal(
    <div className="pointer-events-auto absolute inset-0 z-[1] flex items-center justify-center bg-black/55 p-4 backdrop-blur-[2px] sm:p-6" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)close();}}>
      <aside role="dialog" aria-modal="true" aria-label={`${value.workOrderNumber} reçete detayı`} className="flex max-h-[min(90dvh,880px)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--wms-app-border)] bg-[var(--wms-app-panel-strong)] shadow-2xl" onMouseDown={e=>e.stopPropagation()}>
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--wms-app-border)] bg-[var(--wms-app-panel-strong)] p-5"><div><p className="text-xs font-bold uppercase tracking-widest text-[var(--wms-brand-primary)]">{value.sourceSystemCode} · İş emri / reçete doğrulaması</p><h2 className="mt-1 font-mono text-2xl font-black">{value.workOrderNumber}</h2><p className="mt-1 text-sm text-[var(--wms-app-text-muted)]">{value.productCode} · {value.productName}</p></div><button type="button" onClick={close} className="rounded-lg border border-[var(--wms-app-border)] p-2"><X className="size-5"/></button></header>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          ...
          {value.existingProductionOrderId&&<div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-sm"><strong>Bu {value.sourceSystemCode} iş emri daha önce WMS’e alındı.</strong><div className="mt-1">WMS belgesi: {value.existingProductionDocumentNo}. Yeni WMS emri oluşturulamaz; bağlı transfer hazırlanabilir.</div></div>}
