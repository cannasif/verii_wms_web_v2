...
export function VehicleCheckInPage(){
  ...
  const save=async()=>{if(!form.plateNo.trim()){toast.error('Plaka zorunludur.');return}if(!Number.isInteger(form.steelSheetCount)||form.steelSheetCount<=0){toast.error('Sac levha adedi pozitif tam sayı olmalıdır.');return}setBusy(true);try{const detail=await vehicleCheckInApi.save({...form,branchCode:branch,plateNo:form.plateNo.trim().toUpperCase()});hydrate(detail);setSearchParams({id:String(detail.header.id)},{replace:true});toast.success('Araç giriş kaydı kaydedildi.')}catch(e){toast.error(e instanceof Error?e.message:'Araç kaydı oluşturulamadı.')}finally{setBusy(false)}};
