...
export interface ManualReceiptLine { localId: string; stockId: number; stockCode: string; stockName?: string; yapCodeId?: number; yapCode?: string; quantity: number; unitCode: string; targetWarehouseId: number; targetWarehouseCode?: number; receivingLocationId: number; receivingLocationCode?: string; trackingType: StockTrackingType; lotNo?: string; serialNo?: string; manufacturingDate?: string; expirationDate?: string; scannedBarcode?: string; description?: string; sourceOrderNumber?: string; sourceOrderId?: number }
