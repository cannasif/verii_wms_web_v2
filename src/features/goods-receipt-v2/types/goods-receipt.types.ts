...
export interface ManualGoodsReceiptResult { id: number; documentNo: string; initiationMode: string; status: string; taskId?: number; taskNo?: string; executionId?: number; stockMovementOperationId?: number; qualityInspectionId?: number; lineCount: number; quantity: number; replayed: boolean }
...
export interface GoodsReceiptTaskGridRow { id: number; goodsReceiptId: number; branchCode: string; taskNo: string; documentNo: string; taskType: string; status: string; receiptStatus: string; processType: string; priority: number; warehouseId: number; warehouseCode: number; warehouseName: string; supplierCode?: string; supplierName?: string; lineCount: number; plannedQuantity: number; processedQuantity: number; assigneeCount: number; myAssignmentStatus?: string; plannedStartAtUtc?: string; dueAtUtc?: string; startedAtUtc?: string; completedAtUtc?: string; createdBy?: number; createdDate?: string; updatedBy?: number; updatedDate?: string; rowVersion: string }
