...
export interface ManualReceiptLine { localId: string; stockId: number; stockCode: string; stockName?: string; yapCodeId?: number; yapCode?: string; quantity: number; unitCode: string; targetWarehouseId: number; targetWarehouseCode?: number; receivingLocationId: number; receivingLocationCode?: string; trackingType: StockTrackingType; lotNo?: string; serialNo?: string; manufacturingDate?: string; expirationDate?: string; scannedBarcode?: string; description?: string; sourceOrderNumber?: string; sourceOrderId?: number; requireQualityControl?: boolean }
export interface GoodsReceiptStockQualityRequirement { stockId: number; requiresQualityControl: boolean; source: string; ruleId?: number; inspectionMode: string }
export interface GoodsReceiptQualityRequirementResult { requiresQualityControl: boolean; nextAction: 'SendToQuality' | 'CreateWaybill'; stocks: GoodsReceiptStockQualityRequirement[] }
...
export interface GoodsReceiptDetail { header: GoodsReceiptGridRow; lines: GoodsReceiptDetailLine[]; putawayCandidates: GoodsReceiptPutawayCandidate[]; sourceDocuments: string[]; taskNumbers: string[]; executionCount: number; projectCodes?: string[]; routes?: GoodsReceiptRoutingResult[] }
...
export interface GoodsReceiptTaskGridRow { id: number; goodsReceiptId: number; branchCode: string; taskNo: string; documentNo: string; waybillNo?: string | null; electronicWaybillNo?: string | null; taskType: string; status: string; receiptStatus: string; processType: string; labelStrategy: string; priority: number; warehouseId: number; warehouseCode: number; warehouseName: string; supplierCode?: string; supplierName?: string; lineCount: number; plannedQuantity: number; processedQuantity: number; assigneeCount: number; myAssignmentStatus?: string; plannedStartAtUtc?: string; dueAtUtc?: string; startedAtUtc?: string; completedAtUtc?: string; createdBy?: number; createdDate?: string; updatedBy?: number; updatedDate?: string; rowVersion: string }
...
export interface GoodsReceiptTaskLine { id: number; sequenceNo: number; goodsReceiptLineId: number; stockId: number; stockCode: string; stockName?: string; yapCode?: string; plannedQuantity: number; processedQuantity: number; unitCode: string; status: string; targetWarehouseId: number; toLocationId?: number; trackingType: StockTrackingType; requireQualityControl: boolean; trackings: GoodsReceiptTaskLineTracking[] }
...
export interface GoodsReceiptLabelBatchRow { id:number; goodsReceiptId:number; documentNo:string; waybillNo?:string|null; electronicWaybillNo?:string|null; taskId?:number; taskNo?:string; batchNo:string; status:string; totalLabelCount:number; printedLabelCount:number; consumedLabelCount:number; voidLabelCount:number; lastPrintedAtUtc?:string; createdBy?:number; createdDate?:string; rowVersion:string }
export interface GoodsReceiptLabelRow { id:number; batchId:number; goodsReceiptId:number; goodsReceiptLineId?:number; taskLineId?:number; stockId?:number; stockCode:string; stockName?:string; yapCode?:string; quantity:number; unitCode:string; lotNo?:string; serialNo?:string; manufacturingDate?:string; expirationDate?:string; barcodeValue:string; status:string; printCount:number; lastPrintedAtUtc?:string; consumedAtUtc?:string; voidReason?:string; parentLabelId?:number; rootLabelId?:number; splitAtUtc?:string; splitBy?:number; splitReason?:string; canSplit:boolean; splitBlockReason?:string; rowVersion:string }
export interface GoodsReceiptLabelBatchDetail { batch:GoodsReceiptLabelBatchRow; labels:GoodsReceiptLabelRow[] }
export interface SplitGoodsReceiptLabelResult { source:GoodsReceiptLabelRow; childLabels:GoodsReceiptLabelRow[]; replayed:boolean }
