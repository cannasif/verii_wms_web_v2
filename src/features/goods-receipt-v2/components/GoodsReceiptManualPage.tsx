...
export function GoodsReceiptManualPage({
  ...
  const [selectedRequiresQuality, setSelectedRequiresQuality] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedRequiresQuality(null);
    if (!stockId) return;
    let cancelled = false;
    setTrackingPolicyBusy(true);
    void Promise.all([
      goodsReceiptV2Api.trackingPolicy(policyBranchCode, stockId),
      goodsReceiptV2Api.qualityRequirements(policyBranchCode, [stockId]),
    ])
      .then(([policy, quality]) => {
        if (cancelled) return;
        setTrackingPolicy(policy);
        setSelectedRequiresQuality(quality.requiresQualityControl);
        qualityRequirementCache.current.set(
          `${policyBranchCode}:${stockId}`,
          quality.requiresQualityControl,
        );
      })
      .catch(() => {
        if (cancelled) return;
  ...
  if (result) return <Result result={result} direct={direct} receiptNo={receiptNo} isElectronic={isElectronic} reset={() => { submitIdempotencyKey.current = crypto.randomUUID(); setResult(null); setLines([]); setStep(0); setReceiptNo(''); }} t={t}/>;
  return <section className={cn('wms-ops-form space-y-5', !embedded && 'mx-auto max-w-7xl')}>
    ...
    {step === 2 && <Panel title={t('manual.lines.title')} description={t('manual.lines.description')} icon={<ScanBarcode/>}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="flex items-end"><OpsActionButton type="button" variant="primary" disabled={qualityCheckBusy} onClick={() => addLine()} className="w-full">{qualityCheckBusy?<Loader2 className="size-3.5 shrink-0 animate-spin"/>:<Plus className="size-3.5 shrink-0"/>}{t('manual.addLine')}</OpsActionButton></div>
    ...
    {step === 3 && <Panel title={t('manual.review.title')} description={t('manual.review.description')} icon={<ClipboardCheck/>}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Summary label={t('manual.customer')} value={`${decodeURIComponent(split(customer)[3] || '') || '—'} · ${split(customer)[2] || '—'}`}/><Summary label={t(isElectronic ? 'createFlow.waybill.eReceiptNumber' : 'createFlow.waybill.receiptNumber')} value={receiptNo}/><Summary label={t('manual.documentType')} value={isElectronic ? t('manual.eReceipt') : t('manual.normalReceipt')}/><Summary label={t('manual.documentDate')} value={documentDate}/><Summary label={t('manual.warehouse')} value={`${split(warehouse)[2] || '—'} · ${decodeURIComponent(split(warehouse)[3] || '')}`}/><Summary label={t('manual.lineCount')} value={String(lines.length)}/><Summary label={t('manual.totalQuantity')} value={String(total)}/><Summary label={t('manual.operationType')} value={direct ? t('manual.direct') : t('manual.orderless')}/>{!direct&&<Summary label={t('createFlow.assignees.title')} value={assignees.map(userLabel).join(', ')||'—'}/>}</div><Field label={t('manual.description')}><textarea className={cn(OPS_FIELD_CLASS, 'mt-2 min-h-24 w-full')} maxLength={1000} value={description} onChange={(e) => setDescription(e.target.value)}/></Field></Panel>}

    <footer className="sticky bottom-3 z-20 flex items-center justify-between rounded-2xl border border-[var(--wms-app-border)] bg-[var(--wms-app-panel)]/95 p-3 shadow-xl backdrop-blur"><OpsActionButton type="button" variant="secondary" disabled={step === 0 || busy || qualityCheckBusy} onClick={() => { setError(null); setStep((current) => Math.max(0, current - 1)); }}><ChevronLeft className="size-3.5 shrink-0"/>{t('back')}</OpsActionButton><div className="hidden text-xs text-slate-500 sm:block">{steps[step]?.label}</div>{step < 3 ? <OpsActionButton type="button" variant="primary" disabled={qualityCheckBusy} onClick={() => next()}>{qualityCheckBusy?<Loader2 className="size-3.5 shrink-0 animate-spin"/>:null}{t('continue')}<ChevronRight className="size-3.5 shrink-0"/></OpsActionButton> : <OpsActionButton type="button" variant="primary" disabled={busy} onClick={() => submit()}>{busy ? <Loader2 className="size-3.5 shrink-0 animate-spin"/> : <CheckCircle2 className="size-3.5 shrink-0"/>}{direct ? (lines.some((line)=>line.requireQualityControl)?t('createFlow.sendToQuality'):t('createFlow.createDirect')) : t('manual.createTask')}</OpsActionButton>}</footer>
  </section>;
}
...
function Result({result,direct,receiptNo,isElectronic,reset,t}:{result:ManualGoodsReceiptResult;direct:boolean;receiptNo:string;isElectronic:boolean;reset:()=>void;t:(key:string)=>string}):ReactElement{
  ...
  const printGenerated=async():Promise<void>=>{
    ...
    try{
      ...
      printReceiptLabels(labels,`${receiptNo || `receipt-${result.id}`} ${t('manual.result.printTitleSuffix')}`);
    ...
    <section className="mx-auto max-w-2xl overflow-hidden rounded-3xl border border-emerald-500/35 bg-gradient-to-br from-emerald-500/15 via-[var(--wms-app-panel)] to-transparent shadow-sm">
      <div className="border-b border-emerald-500/20 px-8 py-8 text-center">
        ...
        <div className="mx-auto mt-5 inline-flex rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-5 py-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-slate-500">{t(isElectronic ? 'createFlow.waybill.eReceiptNumber' : 'createFlow.waybill.receiptNumber')}</div>
            <div className="mt-1 font-mono text-xl font-bold text-emerald-700 dark:text-emerald-300">{receiptNo || '—'}</div>
          </div>
        </div>
      </div>
