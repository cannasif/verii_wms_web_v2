...
export function GoodsReceiptListPage():ReactElement{
  ...
  const columns=useMemo<GridColumn<GoodsReceiptGridRow>[]>(()=>{
    ...
    {key:'supplierCode',label:t('list.supplier'),sortable:true,filterable:true,render:r=><><strong>{r.supplierCode||'—'}</strong><div className="text-xs text-slate-500">{r.supplierName}</div></>},
    {key:'warehouseCode',label:t('list.warehouse'),sortable:true,filterable:true,render:r=>`${r.warehouseCode} · ${r.warehouseName}`},
