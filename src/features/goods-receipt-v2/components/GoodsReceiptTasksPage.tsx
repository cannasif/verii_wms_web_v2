...
import type { ActiveUserOption, GoodsReceiptTaskDetail, GoodsReceiptTaskGridRow } from '../types/goods-receipt.types';

export function GoodsReceiptTasksPage({ assignedOnly = false }: { assignedOnly?: boolean }): ReactElement {
  ...
  const pageKey = assignedOnly ? 'goods-receipt-my-tasks' : 'goods-receipt-tasks';
  const refresh = async () => { await queryClient.invalidateQueries({ queryKey: ['advanced-grid', pageKey] }); };
  const open = useCallback(async (row: GoodsReceiptTaskGridRow) => {
    setBusy(row.id);
    try {
      ...
      if (!assignedOnly && users.length === 0) setUsers(await goodsReceiptV2Api.activeUsers());
  ...
  const act = async (kind: 'accept' | 'start') => {
    ...
    try { setDetail(kind === 'accept' ? await goodsReceiptV2Api.acceptTask(detail.task.id) : await goodsReceiptV2Api.startTask(detail.task.id)); toast.success(kind === 'accept' ? 'Emir kabul edildi.' : 'Emir başlatıldı.'); await refresh(); }
  ...
  const saveAssignments = async () => {
    ...
    try { setDetail(await goodsReceiptV2Api.replaceTaskAssignments(detail.task.id, selectedUsers, detail.task.rowVersion)); toast.success('Emir atamaları güncellendi.'); await refresh(); }
    catch (error) { toast.error(message(error, 'Atamalar güncellenemedi.')); }
    finally { setBusy(null); }
  };
  const reload = async () => { if (!detail) return; setDetail(await goodsReceiptV2Api.taskDetail(detail.task.id)); await refresh(); };
  ...
  return <><AdvancedDataGrid pageKey={pageKey} title={assignedOnly ? 'Bana Atanan Mal Kabul Emirleri' : 'Mal Kabul Emir Yönetimi'} description={assignedOnly ? 'Atanmış emirleri kabul edin, başlatın ve barkodla fiziksel kabulü işleyin.' : 'Mal kabul emirlerini, sorumlularını ve ön etiketlerini yönetin.'} columns={columns} fetchPage={assignedOnly ? goodsReceiptV2Api.myTasksPaged : goodsReceiptV2Api.tasksPaged}/>{detail && <TaskModal detail={detail} assignedOnly={assignedOnly} users={users} selectedUsers={selectedUsers} setSelectedUsers={setSelectedUsers} busy={busy === detail.task.id} close={() => setDetail(null)} accept={() => void act('accept')} start={() => void act('start')} save={() => void saveAssignments()} reload={() => void reload()}/>}</>;
}

function TaskModal({ detail, assignedOnly, users, selectedUsers, setSelectedUsers, busy, close, accept, start, save, reload }: { detail: GoodsReceiptTaskDetail; assignedOnly: boolean; users: ActiveUserOption[]; selectedUsers: number[]; setSelectedUsers: (ids: number[]) => void; busy: boolean; close: () => void; accept: () => void; start: () => void; save: () => void; reload:()=>void }): ReactElement {
  const toggle = (id: number) => setSelectedUsers(selectedUsers.includes(id) ? selectedUsers.filter((x) => x !== id) : [...selectedUsers, id]);
  return <ResponsiveDialog onClose={close} title={`Mal kabul emri ${detail.task.taskNo}`} description="Emir kalemleri, atamaları ve fiziksel kabul işlemleri." className="!max-w-6xl">
    <header className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-cyan-500">Mal Kabul Emri</p><h2 className="text-xl font-bold">{detail.task.taskNo}</h2><p className="text-sm text-slate-500">{detail.task.documentNo} · {detail.task.supplierCode} {detail.task.supplierName}</p></div><button type="button" onClick={close} aria-label="Pencereyi kapat" className="grid size-11 shrink-0 place-items-center rounded-xl hover:bg-black/5 dark:hover:bg-white/10"><X className="size-5"/></button></header>
...
function PreLabelPanel({detail}:{detail:GoodsReceiptTaskDetail}):ReactElement {
  const [busy,setBusy]=useState(false);
  const create=async()=>{const lines=detail.lines.filter(x=>x.processedQuantity<x.plannedQuantity).map(x=>({taskLineId:x.id,labelCount:1}));if(lines.length===0){toast.error('Etiket üretilecek açık emir satırı yok.');return}setBusy(true);try{const result=await goodsReceiptV2Api.generateLabels(detail.task.goodsReceiptId,detail.task.id,lines,'Emir ön etiket paketi');toast.success(`${result.batch.totalLabelCount} ön etiket üretildi. Ön Etiketler ekranından yazdırabilirsiniz.`)}catch(error){toast.error(message(error,'Ön etiket üretilemedi.'))}finally{setBusy(false)}};
...
function TaskScanPanel({detail,reload}:{detail:GoodsReceiptTaskDetail;reload:()=>void}):ReactElement {
  ...
  return <section className="mt-5 space-y-4">
    <WarehouseBarcodeScanner branchCode={detail.task.branchCode??'0'} purpose="Inbound" warehouseId={detail.task.warehouseId} expectedStockId={selectedLine?.stockId} disabled={busy}
...
export const GoodsReceiptAssignedTasksPage = (): ReactElement => <GoodsReceiptTasksPage assignedOnly/>;
