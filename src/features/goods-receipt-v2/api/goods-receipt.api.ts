...
export const goodsReceiptV2Api = {
  ...
  splitLabel: async (id:number,payload:{idempotencyKey:string;splitQuantity:number;reason:string;rowVersion:string}):Promise<SplitGoodsReceiptLabelResult> => unwrap(await api.post<Envelope<SplitGoodsReceiptLabelResult>>(`/api/goods-receipts/labels/${id}/split`,payload)),
