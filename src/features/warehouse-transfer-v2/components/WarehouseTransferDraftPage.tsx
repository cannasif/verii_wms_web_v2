...
const requestingUserOption = (user: ActiveUserOption) => {
...
export function WarehouseTransferDraftPage({
  ...
  const [requestedByValue, setRequestedByValue] = useState<string | null>(null);
    ...
    try {
      ...
      const created = variant === "production"
        ? await warehouseTransferApi.createProductionDraft({
            ...
            requestedByUserId: requestedByValue ? Number(requestedByValue.split("|")[0]) : null,
            requestedByName: requestedByValue?.split("|").slice(1).join("|") || null,
      ...
      {variant === "production" && (
        <Panel title={t(`${D}.production.panel`)} icon={<ClipboardList className="size-5" />}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Malzemeyi teslim alacak kişi">
              <div className="wms-ops-field-shell">
                <PagedAppDropdown
                  queryKey={["production-transfer", "requesting-user"]}
                  fetchPage={warehouseTransferApi.activeUsers}
                  toOption={requestingUserOption}
                  value={requestedByValue}
                  onValueChange={setRequestedByValue}
                  minSearchLength={0}
                  pageSize={20}
                  searchFields={["username", "email", "firstName", "lastName"]}
                  sortBy="username"
                  placeholder="Emri isteyen kişiyi seçin"
                  emptyText="Aktif kullanıcı bulunamadı"
                />
              </div>
            </Field>
          </div>
