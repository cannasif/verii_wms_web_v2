...
import type { WarehouseInboundLabelRow } from '../types/warehouse-inbound.types';

const WIDTH_MM=100,HEIGHT_MM=70,SCALE=10;

export function printableLabels(labels:WarehouseInboundLabelRow[]):WarehouseInboundLabelRow[]{
  return labels.filter(x=>!['Void','Consumed'].includes(x.status));
}

export function printReceiptLabels(labels:WarehouseInboundLabelRow[],title:string):void{
...
export async function previewReceiptLabelsPdf(labels:WarehouseInboundLabelRow[],fileName:string):Promise<void>{
...
function labelImage(label:WarehouseInboundLabelRow):string{
