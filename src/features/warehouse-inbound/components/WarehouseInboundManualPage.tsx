...
export function WarehouseInboundManualPage({ direct }: { direct: boolean }): ReactElement {
  ...
  return <section className="mx-auto max-w-7xl space-y-5">
    ...
    {step === 0 && <Panel title={t('manual.document.title')} description={t('manual.document.description')} icon={<FileText/>}><div className="grid gap-5 lg:grid-cols-2">
      ...
      <Field label={t('manual.documentDate')} required><input className="input" type="date" value={documentDate} onChange={(e) => setDocumentDate(e.target.value)}/></Field>
      ...
      {!direct && <Field label={t('manual.plannedArrival')}><input className="input" type="datetime-local" value={plannedArrival} onChange={(e) => setPlannedArrival(e.target.value)}/></Field>}
    </div></Panel>}
    ...
    {step === 2 && <Panel title={t('manual.lines.title')} description={t('manual.lines.description')} icon={<ScanBarcode/>}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <Field label={t('manual.manufacturingDate')}><input className="input" type="date" value={manufacturingDate} onChange={(e) => setManufacturingDate(e.target.value)}/></Field><Field label={t('manual.expirationDate')}><input className="input" type="date" value={expirationDate} onChange={(e) => setExpirationDate(e.target.value)}/></Field>
