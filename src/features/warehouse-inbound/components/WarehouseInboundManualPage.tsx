...
export function WarehouseInboundManualPage({ direct }: { direct: boolean }): ReactElement {
  ...
  return <section className="mx-auto max-w-7xl space-y-5">
    ...
    {step === 0 && <Panel title={t('manual.document.title')} description={t('manual.document.description')} icon={<FileText/>}><div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2 rounded-2xl border border-[var(--wms-app-border)] bg-black/[.025] p-4 dark:bg-white/[.025]"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold">{t('manual.receiptNo')}</h3><p className="text-xs text-slate-500">E-irsaliye / GİB numarası tam 15 alfanümerik karakter olmalıdır.</p></div><label className="flex items-center gap-3 rounded-xl border border-[var(--wms-app-border)] px-4 py-2"><input type="checkbox" checked={isElectronic} disabled readOnly className="size-4 accent-cyan-500"/><span className="text-sm font-semibold">{t('manual.isElectronic')}</span></label></div><div className="relative"><input autoFocus className={`input pr-24 font-mono tracking-wider ${receiptNo && !receiptNoValid ? '!border-red-500' : receiptNoValid ? '!border-emerald-500' : ''}`} inputMode="text" maxLength={15} placeholder="GIB2026AB000000" value={receiptNo} onChange={(e) => { setReceiptNo(normalizeGoodsReceiptDocumentNo(e.target.value)); setError(null); }}/><span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold ${receiptNoValid ? 'text-emerald-500' : 'text-slate-500'}`}>{receiptNo.length}/15</span></div>{receiptNo && <p className={`mt-2 flex items-center gap-1.5 text-xs ${receiptNoValid ? 'text-emerald-500' : 'text-red-500'}`}>{receiptNoValid && <Check className="size-3.5"/>}{receiptNoValid ? t('manual.validNumber') : 'E-irsaliye / GİB numarası tam 15 alfanümerik karakter olmalıdır.'}</p>}</div>
