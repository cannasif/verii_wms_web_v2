...
import type {
  ...
  WarehouseInboundPutawayCandidate,
  LocationOption,
} from '../types/warehouse-inbound.types';

export type WarehouseInboundLifecycleAction = 'approve' | 'shortClose' | 'putaway' | 'cancel';
...
const actionContent: Record<WarehouseInboundLifecycleAction, {
  title: string;
  description: string;
  submit: string;
  icon: ReactElement;
...
export function WarehouseInboundLifecycleDialog({ action, detail, onClose, onCompleted }: Props): ReactElement {
  ...
  const submit = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (action === 'approve') {
        ...
        result = await warehouseInboundV2Api.shortClose(detail.header.id, {
          idempotencyKey: idempotencyKey.current,
          rowVersion: detail.header.rowVersion,
          reason: reason.trim(),
        ...
        result = await warehouseInboundV2Api.putaway(detail.header.id, {
          idempotencyKey: idempotencyKey.current,
          rowVersion: detail.header.rowVersion,
          reason: reason.trim() || undefined,
          ...
          <div className="mt-5 space-y-3">
            ...
            {putawayLines.map((line, index) => (
              <section key={`${line.lineId}:${line.sourceLocationId}:${line.lotNo ?? ''}:${line.serialNo ?? ''}`} className="grid items-end gap-3 rounded-xl border border-[var(--wms-app-border)] p-4 md:grid-cols-[auto_1.4fr_.7fr_1.5fr]">
                ...
                <label className="space-y-1 text-sm"><span className="font-semibold">Hedef raf</span><PagedAppDropdown<LocationOption> disabled={!line.selected} enabled={line.selected} value={line.targetLocationId} onValueChange={(value) => updatePutaway(index, { targetLocationId: value })} queryKey={['gr-putaway-locations', line.warehouseId]} dependencies={[line.warehouseId]} fetchPage={(request) => warehouseInboundV2Api.locations(request, line.warehouseId)} toOption={(item) => ({ value: String(item.id), label: `${item.code} - ${item.name}`, description: item.locationType })} selectedOption={line.defaultTargetLocationId ? { value: String(line.defaultTargetLocationId), label: `Önerilen raf #${line.defaultTargetLocationId}` } : undefined} placeholder="Hedef raf seçin" searchable minSearchLength={1} /></label>
              </section>
            ))}
          </div>
        )}
