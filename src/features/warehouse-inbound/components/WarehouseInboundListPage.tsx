...
import { Ban,CheckCircle2,Eye,FileText,Loader2,PackageCheck,Printer,Warehouse,X } from 'lucide-react';
...
import type { WarehouseInboundDetail,WarehouseInboundGridRow,WarehouseInboundLifecycleResult } from '../types/warehouse-inbound.types';
import { previewReceiptLabelsPdf,printableLabels,printReceiptLabels } from '../utils/warehouse-inbound-label-output';
import { WarehouseInboundLifecycleDialog,type WarehouseInboundLifecycleAction } from './WarehouseInboundLifecycleDialog';
...
export function WarehouseInboundListPage():ReactElement{
  ...
  const openDetail=useCallback(async(id:number)=>{setLoadingId(id);try{setDetail(await warehouseInboundV2Api.detail(id))}catch(error){toast.error(message(error,'Mal kabul detayı açılamadı.'))}finally{setLoadingId(null)}},[]);
  const output=useCallback(async(receiptId:number,lineId:number|undefined,mode:OutputMode,title:string)=>{
    ...
    try{
      const labels=printableLabels(await warehouseInboundV2Api.receiptLabels(receiptId,lineId));
  ...
  const lifecycleCompleted=useCallback(async(result:WarehouseInboundLifecycleResult)=>{
  ...
  return <><AdvancedDataGrid<WarehouseInboundGridRow> key={gridVersion} pageKey="warehouse-inbounds" title="Mal Kabul Kayıtları" description="Mal kabulü görüntüleyin; belge veya kalem bazında etiket yazdırın ve PDF önizleyin." columns={columns} fetchPage={warehouseInboundV2Api.paged}/>{detail&&<DetailModal detail={detail} close={()=>setDetail(null)} output={output} busyKey={outputBusy} onLifecycleCompleted={lifecycleCompleted}/>}</>;
}

function DetailModal({detail,close,output,busyKey,onLifecycleCompleted}:{detail:WarehouseInboundDetail;close:()=>void;output:(receiptId:number,lineId:number|undefined,mode:OutputMode,title:string)=>Promise<void>;busyKey:string;onLifecycleCompleted:(result:WarehouseInboundLifecycleResult)=>Promise<void>}):ReactElement{
  const[action,setAction]=useState<WarehouseInboundLifecycleAction|null>(null);
  ...
  return <ResponsiveDialog onClose={close} title={`Ambar giriş ${detail.header.documentNo}`} description="Ambar giriş bilgileri, kalemleri ve yaşam döngüsü işlemleri." className="!max-w-6xl">
    <header className="flex flex-col items-start justify-between gap-3 sm:flex-row"><div><h2 className="text-xl font-bold">{detail.header.documentNo}</h2><p className="text-sm text-slate-500">{detail.header.supplierCode} · {detail.header.supplierName} · {detail.header.warehouseCode} {detail.header.warehouseName}</p></div><div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end"><button onClick={()=>void output(detail.header.id,undefined,'print',detail.header.documentNo)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold sm:flex-none"><Printer className="size-4"/>Etiketleri Bas</button><button onClick={()=>void output(detail.header.id,undefined,'pdf',detail.header.documentNo)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold sm:flex-none"><FileText className="size-4"/>PDF Göster</button><button onClick={close} aria-label="Pencereyi kapat" className="grid size-11 shrink-0 place-items-center rounded-xl hover:bg-black/5 dark:hover:bg-white/10"><X className="size-5"/></button></div></header>
    ...
    <div className="mt-5 overflow-x-auto rounded-xl border border-[var(--wms-app-border)]"><table className="w-full text-sm"><thead className="bg-black/5 text-left dark:bg-white/5"><tr><th className="p-3">#</th><th className="p-3">Stok</th><th className="p-3">YAP</th><th className="p-3 text-right">Beklenen</th><th className="p-3 text-right">Alınan</th><th className="p-3">Durum</th><th className="p-3 text-center">Etiket</th></tr></thead><tbody>{detail.lines.map(line=><tr key={line.id} className="border-t border-[var(--wms-app-border)]"><td className="p-3">{line.lineNo}</td><td className="p-3"><strong>{line.stockCode}</strong><div className="text-xs text-slate-500">{line.stockName}</div></td><td className="p-3">{line.yapCode||'—'}</td><td className="p-3 text-right">{formatProjectNumber(line.expectedQuantity)}</td><td className="p-3 text-right">{formatProjectNumber(line.receivedQuantity)}</td><td className="p-3">{line.status}</td><td className="p-3"><div className="flex justify-center gap-1"><ActionButton title="Kalem etiketini yazdır" busy={busyKey===`${detail.header.id}:${line.id}:print`} onClick={()=>void output(detail.header.id,line.id,'print',`${detail.header.documentNo}-${line.lineNo}`)} icon={<Printer className="size-4"/>}/><ActionButton title="Kalem etiketini PDF göster" busy={busyKey===`${detail.header.id}:${line.id}:pdf`} onClick={()=>void output(detail.header.id,line.id,'pdf',`${detail.header.documentNo}-${line.lineNo}`)} icon={<FileText className="size-4"/>}/></div></td></tr>)}</tbody></table></div>
    {action&&<WarehouseInboundLifecycleDialog action={action} detail={detail} onClose={()=>setAction(null)} onCompleted={async(result)=>{setAction(null);await onLifecycleCompleted(result)}}/>}
  </ResponsiveDialog>;
}
