...
import type {
  ActiveUserOption,
  WarehouseInboundTaskDetail,
  WarehouseInboundTaskGridRow,
} from "../types/warehouse-inbound.types";

export function WarehouseInboundTasksPage({
  assignedOnly = false,
}: {
  ...
  const open = useCallback(
    async (row: WarehouseInboundTaskGridRow) => {
      setBusy(row.id);
      try {
        ...
        if (!assignedOnly && users.length === 0)
          setUsers(await warehouseInboundV2Api.activeUsers());
      } catch (error) {
  ...
  const act = async (kind: "accept" | "start") => {
    ...
    try {
      setDetail(
        kind === "accept"
          ? await warehouseInboundV2Api.acceptTask(detail.task.id)
          : await warehouseInboundV2Api.startTask(detail.task.id),
      );
      toast.success(
  ...
  const saveAssignments = async () => {
    ...
    try {
      setDetail(
        await warehouseInboundV2Api.replaceTaskAssignments(
          detail.task.id,
          selectedUsers,
  ...
  const reload = async () => {
    if (!detail) return;
    setDetail(await warehouseInboundV2Api.taskDetail(detail.task.id));
    await refresh();
  };
  ...
  return (
    <>
      <AdvancedDataGrid
        ...
        fetchPage={
          assignedOnly
            ? warehouseInboundV2Api.myTasksPaged
            : warehouseInboundV2Api.tasksPaged
        }
      />
      {detail && (
        <TaskModal
...
function TaskModal({
  ...
  detail: WarehouseInboundTaskDetail;
  assignedOnly: boolean;
  ...
  return (
    <ResponsiveDialog
      onClose={close}
      title={`Ambar giriş emri ${detail.task.taskNo}`}
    ...
    >
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        <Info label="Durum" value={localizeEnumValue(detail.task.status)} />
        <Info
          label="Depo"
...
function PreLabelPanel({
  ...
  const create = async () => {
    ...
    setBusy(true);
    try {
      const result = await warehouseInboundV2Api.generateLabels(
        detail.task.warehouseInboundId,
        detail.task.id,
        lines,
...
function TaskScanPanel({
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const result = await warehouseInboundV2Api.receiveTaskScan(
        detail.task.id,
        {
    ...
    <section className="mt-5 space-y-4">
      ...
      <section className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold">Çözümlenen ambar giriş kaydı</h3>
            <p className="text-xs text-slate-500">
...
function processTypeLabel(value: string): string {
  return localizeEnumValue(value);
}
function message(error: unknown, fallback: string): string {
  return error instanceof Error ? error.message : fallback;
}
export const WarehouseInboundAssignedTasksPage = (): ReactElement => (
  <WarehouseInboundTasksPage assignedOnly />
);
