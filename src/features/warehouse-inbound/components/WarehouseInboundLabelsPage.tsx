...
import type {
  WarehouseInboundLabelBatchDetail,
  WarehouseInboundLabelBatchRow,
...
export function WarehouseInboundLabelsPage(): ReactElement {
  ...
  const open = useCallback(async (id: number) => {
    setBusy(id);
    try {
      setDetail(await warehouseInboundV2Api.labelBatch(id));
    } catch (error) {
  ...
  const columns = useMemo<GridColumn<WarehouseInboundLabelBatchRow>[]>(
    () => [
  ...
  const reload = async () => {
    if (detail)
      setDetail(await warehouseInboundV2Api.labelBatch(detail.batch.id));
    await cache.invalidateQueries({
      queryKey: ["advanced-grid", "warehouse-inbound-labels"],
    });
  };
  return (
...
function LabelDialog({
  ...
  detail: WarehouseInboundLabelBatchDetail;
  close: () => void;
  reload: () => void;
}): ReactElement {
  ...
  const print = async () => {
    ...
    try {
      ...
      win.document.close();
      await warehouseInboundV2Api.markLabelsPrinted(printable.map((x) => x.id));
  ...
  const voidLabel = async (label: WarehouseInboundLabelRow) => {
    const reason = window.prompt("Etiket iptal nedenini yazın:");
    if (!reason?.trim()) return;
    setBusy(true);
    try {
      await warehouseInboundV2Api.voidLabel(
        label.id,
        reason.trim(),
        label.rowVersion,
      );
            ...
            {detail.labels.map((label) => (
              ...
              >
                ...
                <td className="p-3 text-center">
                  {localizeEnumValue(label.status)}
                </td>
                <td className="p-3 text-center">
                  <button
                    type="button"
...
function labelHtml(label: WarehouseInboundLabelRow): string {
