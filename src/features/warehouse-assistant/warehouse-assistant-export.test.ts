...
): WarehouseAssistantExportTranslator {
  return (key, options) => {
  ...
  activities: [
    {
      id: 1,
      action: "Update",
      description: "Raf güncellendi",
      entityType: "Location",
      entityId: "74",
      result: "Success",
      userId: 2,
      userDisplayName: "Depo Kullanıcısı",
      occurredAtUtc: "2026-08-08T08:30:00Z",
    },
  ],
  serialBalances: [
    {
  ...
  movements: [
    {
      ...
      serialNo: "DTG-1",
      stockStatus: "Available",
      occurredAtUtc: "2026-08-08T10:00:00Z",
      isReversal: false,
    },
  ],
  tasks: [
    {
      module: "ProductionTransfer",
      taskId: 11,
      taskNo: "TASK-1",
      taskType: "Picking",
      status: "Assigned",
      ...
      processedQuantity: 1,
      remainingQuantity: 2,
      plannedAtUtc: "2026-08-08T07:00:00Z",
      dueAtUtc: "2026-08-09T07:00:00Z",
      assigneeUserId: 2,
      assigneeDisplayName: "Depo Kullanıcısı",
    },
  ],
  goodsReceipts: [
    {
      ...
      receivedAtUtc: "2026-08-08T10:30:00Z",
      supplierId: 20,
      supplierCode: "ABC",
      ...
      putawayQuantity: 9,
      status: "Completed",
      qualityStatus: "NotRequired",
      erpIntegrationStatus: "Posted",
      receivedByUserId: 2,
      receivedByDisplayName: "Depo Kullanıcısı",
    },
  ],
  steelVehicles: [
    {
      vehicleCheckInId: 31,
      plateNo: "34 ABC 123",
      trailerPlateNo: "34 XYZ 987",
      driverName: "Ali Test",
      carrierName: "Test Nakliye",
      declaredSteelSheetCount: 12,
      acceptedPlateCount: 11,
      unresolvedPlateCount: 1,
      status: "ContainsUnknownPlates",
      checkedInAtUtc: "2026-08-08T06:45:00Z",
      businessDate: "2026-08-08",
      customerCode: "ABC",
      customerName: "ABC TEDARIK",
    },
  ],
  transfers: [
    {
      ...
      sourceWarehouseName: "Ana Depo",
      targetWarehouseCode: 2,
      targetWarehouseName: "Üretim Alanı",
      status: "CompletedWithShortage",
      ...
      receivedQuantity: 8,
      putawayQuantity: 8,
      shortClosedQuantity: 1,
      externalReferenceNo: "WO-2026-41",
...
describe("warehouse assistant export model", () => {
  ...
  it("creates localized metadata and separate result sections without losing numeric/date types", () => {
    ...
    expect(model.metadata[0]).toEqual({
      label: "Kullanıcı sorusu",
      value: "DTG-1 hareketlerini göster",
    });
    expect(model.metadata[2]).toEqual({
      label: "Yetki kapsamı",
    ...
    expect(
      model.sections.find((item) => item.key === "movements")?.rows[0]
        .occurredAt,
    ).toBeInstanceOf(Date);
    expect(
      model.sections.find((item) => item.key === "serial-balances")?.rows[0]
    ...
    expect(
      model.sections.find((item) => item.key === "goods-receipts")?.rows[0]
        .status,
    ).toBe(translator(tr)("operationStatuses.Completed"));
    expect(
      model.sections.find((item) => item.key === "steel-vehicles")?.rows[0]
        .status,
    ).toBe(translator(tr)("vehicleStatuses.ContainsUnknownPlates"));
    expect(
      model.sections.find((item) => item.key === "transfers")?.rows[0]
        .businessContext,
    ).toBe(translator(tr)("transferContexts.ProductionMaterialSupply"));
    expect(
      model.sections.find((item) => item.key === "transfers")?.rows[0]
        .shortClosedQuantity,
    ).toBe(1);
  ...
  it("keeps PDF tables limited to readable operational columns", () => {
    ...
    model.sections.forEach((item) => {
      const pdfColumns = item.columns.filter(
        (column) => column.includeInPdf !== false,
      );
      expect(pdfColumns.length).toBeGreaterThan(0);
      expect(pdfColumns.length).toBeLessThanOrEqual(10);
    });
    expect(model.sections.find((item) => item.key === "tasks")?.title).toBe(
