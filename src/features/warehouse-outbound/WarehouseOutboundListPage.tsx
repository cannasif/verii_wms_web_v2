...
export function WarehouseOutboundListPage() {
  ...
  const columns = useMemo<GridColumn<ShipmentGridRow>[]>(() => [
    ...
    { key: 'status', label: 'Durum', sortable: true, filterable: true, render: (row) => <OpsStatusBadge tone={inferOpsStatusTone(row.status)}>{row.status}</OpsStatusBadge> },
    ...
    { key: 'shippedQuantity', label: 'Sevk', sortable: true, filterable: true, render: (row) => formatProjectNumber(row.shippedQuantity) },
    {
      ...
      render: (row) => <div className="flex items-center gap-1">
        {row.status !== 'Cancelled' && <Link to={`/warehouse/warehouse-outbounds/${row.id}/operations`} title="Operasyonu yürüt" className="rounded-lg p-2 text-cyan-500 hover:bg-cyan-500/10"><PlayCircle className="size-4" /></Link>}
  ...
  return <>
    <AdvancedDataGrid key={revision} pageKey="warehouse-outbounds" title="Sevk Kayıtları"
...
function LifecycleDialog({ value, close, completed }: { value: { row: ShipmentGridRow; kind: 'delete' | 'cancel' }; close: () => void; completed: () => void }) {
  ...
  const title = value.kind === 'delete' ? 'Taslağı sil' : 'Ambar çıkışını iptal et';
