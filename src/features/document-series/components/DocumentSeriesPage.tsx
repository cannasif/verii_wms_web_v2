...
export function DocumentSeriesPage() {
  ...
  const columns = useMemo<GridColumn<DocumentSeriesRow>[]>(() => moduleReady ? [
    ...
    { key: 'warehouseName', label: t('columns.warehouse'), render: (row) => row.warehouseCode == null ? t('form.allWarehouses') : `${row.warehouseCode} - ${row.warehouseName ?? ''}` },
    { key: 'code', label: t('columns.code'), render: (row) => <span className="font-mono font-semibold">{row.code}</span> },
    { key: 'name', label: t('columns.name'), render: (row) => row.name },
    { key: 'documentType', label: t('columns.documentType'), render: (row) => t(`types.${row.documentType}`) },
    { key: 'prefix', label: t('columns.prefix'), render: (row) => row.prefix },
    ...
    { key: 'lastIssuedAt', label: t('columns.lastIssuedAt'), render: (row) => row.lastIssuedAt ? formatProjectDateTime(row.lastIssuedAt) : '-' },
