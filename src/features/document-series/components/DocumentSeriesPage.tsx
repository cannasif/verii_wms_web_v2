...
import { FileDigit, Loader2, Pencil, Plus, Trash2 } from 'lucide-react';
...
export function DocumentSeriesPage() {
  ...
  return <>
    ...
    {mode && <Dialog open onOpenChange={(open) => { if (!open && !saving) setMode(null); }}>
      <OpsDialogContent size="xl">
        <OpsDialogHeader>
          <div className="flex items-center gap-3">
            <div className="grid size-11 place-items-center rounded-xl bg-[var(--wms-brand-primary)] text-white"><FileDigit className="size-5"/></div>
            <div>
              ...
              <DialogDescription className="text-sm text-slate-500">{t('form.description')}</DialogDescription>
            </div>
          </div>
        </OpsDialogHeader>
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          ...
          <OpsDialogFooter>
            <button type="button" disabled={saving} onClick={() => setMode(null)} className="rounded-xl border px-5 py-2.5">{t('form.cancel')}</button>
            <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[var(--wms-brand-primary)] px-5 py-2.5 font-semibold text-white disabled:opacity-50">{saving ? <Loader2 className="size-4 animate-spin"/> : mode === 'create' ? <Plus className="size-4"/> : <Pencil className="size-4"/>}{mode === 'create' ? t('form.create') : t('form.save')}</button>
          </OpsDialogFooter>
        </form>
      </OpsDialogContent>
    </Dialog>}
