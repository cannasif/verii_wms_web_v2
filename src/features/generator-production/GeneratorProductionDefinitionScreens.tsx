...
import { AlertTriangle, CalendarDays, ChevronRight, Clock3, Factory, GitBranch, Loader2, Route, Save, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, UsersRound, Wrench } from 'lucide-react';
...
export function GeneratorProductionParametersPage(): ReactElement {
  ...
  return <section className="space-y-5">
    <GeneratorPageHeader eyebrow="Jeneratör Üretim / Tanımlar" title="Üretim Parametreleri" description="Kod değişikliği yapmadan proje varsayılanlarını, planlama pencerelerini ve yürütme kontrollerini yönetin." actions={<button className={gpPrimaryButton} disabled={save.isPending} onClick={() => save.mutate(form)}>{save.isPending ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />} Parametreleri kaydet</button>} />
    ...
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <PolicySection code="GP.POL.PLAN" title="Planlama politikası" description="Plan motorunun sıralama, takvim ve görünüm sınırları.">
        ...
        <PolicyNumber label="Azami takvim aralığı" hint="Gün" value={form.maximumScheduleRangeDays} min={1} max={3660} onChange={(value) => set('maximumScheduleRangeDays', value)} />
        <PolicyNumber label="Geçmiş plan penceresi" hint="Gün" value={form.schedulePastDays} min={0} max={form.maximumScheduleRangeDays} onChange={(value) => set('schedulePastDays', value)} />
        <PolicyNumber label="Gelecek plan penceresi" hint="Gün" value={form.scheduleFutureDays} min={1} max={form.maximumScheduleRangeDays} onChange={(value) => set('scheduleFutureDays', value)} />
      ...
      <PolicySection code="GP.POL.EXEC" title="Yürütme politikası" description="İstasyon operasyonlarının güvenli durum geçişleri.">
        <PolicyNumber label="Operasyon gerekçesi asgari uzunluk" hint="Karakter" value={form.minimumOperationReasonLength} min={3} max={1000} onChange={(value) => set('minimumOperationReasonLength', value)} />
        <PolicyNumber label="Andon yenileme sıklığı" hint="Saniye" value={form.andonRefreshSeconds} min={5} max={3600} onChange={(value) => set('andonRefreshSeconds', value)} />
        <PolicyNumber label="Takvim arama güvenlik sınırı" hint="Gün" value={form.workingCalendarSearchLimitDays} min={1} max={36600} onChange={(value) => set('workingCalendarSearchLimitDays', value)} />
        <PolicyToggle label="Final montaj için bileşen zorunlu" checked={form.requireComponentForFinalAssembly} onChange={(value) => set('requireComponentForFinalAssembly', value)} />
        <PolicyToggle label="Başlatmada malzeme uygunluğu zorunlu" checked={form.requireMaterialAvailabilityToStart} onChange={(value) => set('requireMaterialAvailabilityToStart', value)} />
        <PolicyToggle label="Tamamlamada açık problem olmamalı" checked={form.requireProblemClosureToComplete} onChange={(value) => set('requireProblemClosureToComplete', value)} />
        <PolicyToggle label="Tamamlamada pozitif miktar zorunlu" checked={form.requirePositiveCompletionQuantity} onChange={(value) => set('requirePositiveCompletionQuantity', value)} />
      </PolicySection>
    </div>
