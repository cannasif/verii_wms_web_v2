...
export const procurementApi={
  ...
  policy:async():Promise<ProcurementPolicy>=>unwrap(await api.get<Envelope<ProcurementPolicy>>('/api/procurement/policy')),
  updatePolicy:async(payload:Omit<ProcurementPolicy,'id'|'branchCode'|'updatedBy'|'updatedDate'>):Promise<ProcurementPolicy>=>unwrap(await api.put<Envelope<ProcurementPolicy>>('/api/procurement/policy',payload,{useNativeHttpMethod:true})),
  convertRequestToRfq:async(id:number,payload:{responseDueDate:string;supplierIds:number[];buyerMessage?:string;lines:RfqRequestLineInput[]}):Promise<number>=>unwrap<{rfqId:number}>(await api.post<Envelope<{rfqId:number}>>(`/api/procurement/requests/${id}/convert-to-rfq`,payload)).rfqId,
