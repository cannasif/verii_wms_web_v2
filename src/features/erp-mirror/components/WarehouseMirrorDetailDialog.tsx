...
import { Building2, Hash, RefreshCw, ShieldCheck, Warehouse } from 'lucide-react';
...
import type { WarehouseMirror } from '../types/erp-mirror.types';

const CARD = 'erpMirror.warehouseCardUi';
...
export function WarehouseMirrorDetailDialog({
  warehouse,
  onClose,
}: {
  warehouse: WarehouseMirror | null;
  onClose: () => void;
}): ReactElement {
  const { t } = useTranslation('common');

  return (
    <Dialog open={Boolean(warehouse)} onOpenChange={open => { if (!open) onClose(); }}>
      <OpsDialogContent
        size="xl"
        portalRoot="body"
          ...
          <div className="flex w-full items-start gap-3 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <DialogDescription className="wms-ops-detail-dialog__description">
                {warehouse?.warehouseName || t(`${CARD}.untitled`)}
              </DialogDescription>
              {warehouse ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  <OpsCodeBadge>{display(warehouse.warehouseCode)}</OpsCodeBadge>
                  <span className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--wms-app-border)] bg-[color-mix(in_oklab,var(--wms-app-panel)_80%,transparent)] px-2.5 py-1 text-xs font-semibold text-[var(--wms-app-text-muted)]">
                    ...
                    {t('erpMirror.fields.branchCode')}: {display(warehouse.branchCode)}
                  </span>
            ...
            <div className="space-y-5">
              <section className="overflow-hidden rounded-xl border border-[color-mix(in_oklab,var(--wms-ops-accent)_22%,var(--wms-app-border))] bg-[linear-gradient(135deg,color-mix(in_oklab,var(--wms-ops-accent)_10%,transparent),transparent_55%)]">
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[color-mix(in_oklab,var(--wms-ops-accent)_14%,transparent)] px-4 py-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-sm text-[var(--wms-app-text-muted)]">{warehouse.warehouseName}</p>
                  </div>
                  <div className="text-right text-xs text-[var(--wms-app-text-muted)]">
                    ...
                    <p className="mt-1 font-mono font-semibold text-[var(--wms-app-text)]">
                      {warehouse.lastSyncDate ? formatProjectDateTime(warehouse.lastSyncDate) : '—'}
                    </p>
                  </div>
                </div>
                <div className="grid gap-px bg-[color-mix(in_oklab,var(--wms-app-border)_70%,transparent)] sm:grid-cols-2">
                  <MetaCell label={t('erpMirror.fields.branchCode')} value={display(warehouse.branchCode)} />
                  <MetaCell label={t('erpMirror.fields.warehouseCode')} value={display(warehouse.warehouseCode)} />
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="grid gap-3 rounded-xl border border-[var(--wms-app-border)] bg-[color-mix(in_oklab,var(--wms-app-panel)_92%,transparent)] p-4 sm:grid-cols-2">
                  <OpsDetailField label={t('erpMirror.fields.id')}>{display(warehouse.id)}</OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.branchCode')}>{display(warehouse.branchCode)}</OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.warehouseCode')}>{display(warehouse.warehouseCode)}</OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.warehouseName')} wide>
                    {display(warehouse.warehouseName)}
                  </OpsDetailField>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                />
                <div className="grid gap-3 rounded-xl border border-[var(--wms-app-border)] bg-[color-mix(in_oklab,var(--wms-app-panel)_92%,transparent)] p-4 sm:grid-cols-2">
                  <OpsDetailField label={t('erpMirror.fields.createdBy')}>{display(warehouse.createdBy)}</OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.createdDate')}>
                    {warehouse.createdDate ? formatProjectDateTime(warehouse.createdDate) : '—'}
                  </OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.updatedBy')}>{display(warehouse.updatedBy)}</OpsDetailField>
                  <OpsDetailField label={t('erpMirror.fields.updatedDate')}>
                    {warehouse.updatedDate ? formatProjectDateTime(warehouse.updatedDate) : '—'}
                  </OpsDetailField>
                </div>
              </section>
            </div>
