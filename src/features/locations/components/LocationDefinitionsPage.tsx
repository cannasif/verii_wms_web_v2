  ...
  return <>
    ...
    {mode && (
      <Dialog open onOpenChange={(open) => { if (!open && !saving) setMode(null); }}>
        <OpsDialogContent size="full">
          <OpsDialogHeader>
            <div className="flex items-center gap-3">
              <div className="grid size-11 place-items-center rounded-xl bg-[var(--wms-brand-primary)] text-white">{mode === 'create' ? <Plus className="size-5"/> : <Pencil className="size-5"/>}</div>
              <div>
                <DialogTitle className="wms-ops-detail-dialog__title text-xl font-bold">{mode === 'create' ? t('form.createTitle') : t('form.editTitle')}</DialogTitle>
          ...
          {loadingForm ? (
            ...
            <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
              ...
              <OpsDialogFooter>
                <OpsActionButton type="button" variant="secondary" disabled={saving} onClick={() => setMode(null)}>{t('form.cancel')}</OpsActionButton>
                <OpsActionButton type="submit" variant="primary" disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin"/> : null}{mode === 'create' ? t('form.create') : t('form.save')}</OpsActionButton>
              </OpsDialogFooter>
            </form>
          )}
        </OpsDialogContent>
    ...
    <DeleteConfirmDialog
      ...
      description={deleteTarget ? t('deleteDialog.description', { name: `${deleteTarget.code} - ${deleteTarget.name}` }) : undefined}
  ...
  return (
    <div className="wms-ops-form-item">
      <label className="wms-ops-prelabel-form-label">
        {label}{required && <span className="wms-ops-required"> *</span>}
      </label>
      {children}
    </div>
  );
}
function NumberInput({ value, set }: { value: string; set: (value: string) => void }) {
  return <AppInput type="number" min={0} max={9999} value={value} onChange={(e) => set(e.target.value)} />;
}
