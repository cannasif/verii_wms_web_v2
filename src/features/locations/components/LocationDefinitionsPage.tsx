          ...
          {loadingForm ? (
            ...
            <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
              <OpsDialogBody className="space-y-6">
                ...
                <Field label={t('form.descriptionLabel')}><textarea value={form.description} maxLength={500} rows={3} onChange={(e) => update('description', e.target.value)} className="input min-h-[5rem] resize-y"/></Field>
              </OpsDialogBody>
              <OpsDialogFooter>
                <button type="button" disabled={saving} onClick={() => setMode(null)} className="rounded-xl border px-5 py-2.5 disabled:opacity-50">{t('form.cancel')}</button>
                <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[var(--wms-brand-primary)] px-5 py-2.5 font-semibold text-white disabled:opacity-50">{saving && <Loader2 className="size-4 animate-spin"/>}{mode === 'create' ? t('form.create') : t('form.save')}</button>
              </OpsDialogFooter>
            </form>
          )}
        </OpsDialogContent>
