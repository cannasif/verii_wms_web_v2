...
const locationCodePattern = /^[A-Z0-9][A-Z0-9._/-]{0,49}$/;
...
export function LocationDefinitionsPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    else if (!locationCodePattern.test(form.code.trim().toUpperCase())) validationError = t('validation.invalidCode');
...
function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: string; children: ReactNode }) {
  return (
    <div className="wms-ops-form-item">
      ...
      {children}
      {hint && <small className="mt-1 block text-xs text-slate-500 dark:text-slate-400">{hint}</small>}
    </div>
  );
}
