...
export function InspectionDialog({row,close,done}:{row:SteelLineRow;close:()=>void;done:()=>void}){
  ...
  return <Dialog open onOpenChange={v=>{if(!v)close()}}><DialogContent className="!max-w-2xl" data-no-auto-localize="true"><DialogTitle>{row.dCode} · {row.supplierSerialNo}</DialogTitle>
    ...
    <div className="flex justify-end gap-3"><button onClick={close} className="rounded-xl border px-4 py-2">{t(`${D}.cancel`)}</button><button disabled={busy||!!validationError} onClick={()=>void save()} className="rounded-xl bg-cyan-600 px-4 py-2 font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{t(`${D}.saveDecision`)}</button></div>
