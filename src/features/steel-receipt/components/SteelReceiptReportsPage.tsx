...
export function SteelReceiptReportsPage(): ReactElement {
  ...
  const columns = useMemo<GridColumn<SteelLineRow>[]>(() => [
    ...
    { key: 'stockCode', label: 'Stok', sortable: true, filterable: true, render: row => <><strong>{row.stockCode}</strong><small className="block text-slate-500">{row.stockName}</small></> },
