...
function ReceiptPanel(){
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,.7fr)]">
    <section className="overflow-hidden rounded-2xl border bg-[var(--wms-app-surface)]"><SectionHead title="Onaylı Levha Adayları" text="Kalite kararı tamamlanmış, henüz ortak mal kabule aktarılmamış levhalar."/>
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="border-y text-left text-xs uppercase text-slate-500"><th className="p-3">Seç</th><th>DCode / Seri</th><th>Stok</th><th>Plan</th><th>Kalite</th><th className="text-right">Onaylı Miktar</th></tr></thead><tbody>{rows.map(row=><tr key={row.id} className="border-b last:border-0"><td className="p-3"><input type="checkbox" checked={!!selected[row.id]} onChange={()=>toggle(row)} className="size-4 accent-cyan-500"/></td><td><strong className="font-mono text-cyan-500">{row.dCode}</strong><small className="block">{row.supplierSerialNo}</small></td><td><strong>{row.stockCode}</strong><small className="block text-slate-500">{row.stockName}</small></td><td>{row.importReferenceNo}<small className="block">{row.netsisOrderNo||'Siparişsiz'}</small></td><td>{localizeEnumValue(row.inspectionStatus)}</td><td className="text-right font-bold">{formatProjectNumber(row.approvedQuantity)} {row.unitCode}</td></tr>)}</tbody></table></div>
...
function PlacementPanel(){
  ...
  const place=async()=>{if(!selected||!location){toast.error('Levha ve hedef raf seçilmelidir.');return}setBusy(true);try{const result=await steelReceiptApi.place(selected.id,{locationId:Number(location),rowVersion:selected.rowVersion});toast.success(`${selected.dCode}, ${result.stackOrderNo}. istif olarak üst üste yerleştirildi.`);setSelected(null);setLocation(null);await cache.invalidateQueries({queryKey:['steel-placement-candidates']});await cache.invalidateQueries({queryKey:['steel-occupancy']})}catch(e){toast.error(e instanceof Error?e.message:'Yerleştirme tamamlanamadı.')}finally{setBusy(false)}};
    ...
    <section className="space-y-4 rounded-2xl border bg-[var(--wms-app-surface)] p-5"><SectionHead title="Raf Doluluğu ve Yerleşim" text={selected?`${selected.dCode} · ${selected.stockCode}`:'Devam etmek için soldan levha seçin.'}/>
      ...
      {location&&<><div className="grid gap-4 lg:grid-cols-[.8fr_1.2fr]">
        <section className="overflow-hidden rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-transparent to-violet-500/10 p-5">
          <div className="flex items-center justify-between"><div><span className="text-xs font-bold uppercase tracking-widest text-cyan-500">Otomatik Yerleşim</span><h3 className="mt-1 text-xl font-black">Üst Üste İstif</h3></div><Layers3 className="size-10 text-cyan-500"/></div>
          <div className="mt-5 grid grid-cols-2 gap-3"><Metric label="Raftaki levha" value={String(occupancy.data?.length??0)}/><Metric label="Yeni istif sırası" value={String(nextStack)}/></div>
          <p className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-600">İstif numarası API tarafından transaction içinde yeniden hesaplanır. Eş zamanlı kullanıcılar aynı sırayı alamaz.</p>
        </section>
        <SteelStackVisual items={occupancy.data??[]} pendingCode={selected.dCode} nextStack={nextStack}/>
      </div>
      <div><strong className="text-sm">Seçili raftaki istif sırası ({occupancy.data?.length??0})</strong><div className="mt-2 grid gap-2 md:grid-cols-2">{[...(occupancy.data??[])].sort((a,b)=>(b.stackOrderNo??0)-(a.stackOrderNo??0)).map(item=><div key={item.placementId} className="rounded-xl border p-3 text-xs"><strong>{item.stackOrderNo}. istif · {item.dCode}</strong><span className="block text-slate-500">{item.stockCode} · {item.supplierSerialNo}</span></div>)}</div>{!occupancy.isLoading&&!occupancy.data?.length&&<p className="mt-2 text-xs text-slate-500">Bu raf boş; seçilen levha 1. istif olacaktır.</p>}</div>
      <button disabled={busy||occupancy.isLoading} onClick={()=>void place()} className="w-full rounded-xl bg-cyan-600 px-4 py-3 font-bold text-white disabled:opacity-40"><Layers3 className="mr-2 inline size-4"/>{nextStack}. İstif Olarak Yerleştir</button></>}</>}</section></div>;
}
