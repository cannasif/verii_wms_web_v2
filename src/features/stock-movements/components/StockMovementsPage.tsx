        ...
        <OpsDialogContent size="xl" className="data-no-auto-localize">
          ...
          <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
            <OpsDialogBody className="space-y-5">
              ...
              {needsTarget && <section className="rounded-xl border p-4"><h3 className="mb-3 flex items-center gap-2 font-semibold text-emerald-600"><ArrowDownToLine className="size-4" />{t(`${P}.targetSection`)}</h3><div className="grid gap-4 sm:grid-cols-2"><Field label={t(`${P}.targetWarehouse`)}><AppDropdown value={form.targetWarehouseId} onValueChange={(value) => void selectWarehouse('target', value)} options={warehouses.map((item) => ({ value: String(item.id), label: `${item.warehouseCode} - ${item.warehouseName}` }))} placeholder={t(`${P}.selectWarehouse`)} ariaLabel={t(`${P}.targetWarehouse`)} searchable portalContainer={null} /></Field><Field label={t(`${P}.targetLocation`)}><AppDropdown value={form.targetLocationId} onValueChange={(value) => setValue('targetLocationId', value)} options={targetLocations.map((item) => ({ value: String(item.id), label: `${item.code} - ${item.name}` }))} placeholder={t(`${P}.selectLocation`)} ariaLabel={t(`${P}.targetLocation`)} searchable portalContainer={null} /></Field></div></section>}
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <Field label={t(`${P}.yapCode`)}><AppDropdown value={form.yapCodeId} onValueChange={(value) => setValue('yapCodeId', value)} options={yapCodes.filter(item => !item.stockId || String(item.stockId) === form.stockId).map((item) => ({ value: String(item.id), label: item.configurationCode, description: item.description || undefined }))} placeholder={t(`${P}.selectYapCodeOptional`)} ariaLabel={t(`${P}.yapCode`)} searchable portalContainer={null} /></Field>
                ...
                <Field label={t(`${P}.stockStatus`)}><AppDropdown value={form.stockStatus} onValueChange={(value) => setValue('stockStatus', value)} options={stockStatusOptions} ariaLabel={t(`${P}.stockStatus`)} portalContainer={null} /></Field>
