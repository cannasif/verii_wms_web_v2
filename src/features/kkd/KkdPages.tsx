...
export function KkdPolicyPage() {
  ...
  return <Page title="KKD Süreç Politikası" description="Şube bazında dağıtım ön koşullarını yönetin; değişiklikler yeni dağıtımlarda servis katmanında zorunlu uygulanır.">
    <div className={`${panel} space-y-3`}>
      ...
      <PolicyRow icon={<ShieldCheck className="size-5 text-cyan-500"/>} title="Kota aşımında yönetici fiziksel onayı" text="Açık olduğunda hak üstü KKD için ambar çıkışı, yetkili yönetici onayı gelmeden serbest bırakılamaz." control={toggle('requireManagerApprovalForExcess')}/>
  ...
  return <Page title="KKD Tanımları" description="V1 kurallarını tek matris motorunda, tarih ve yaşam döngüsü fazlarıyla yönetin.">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(340px,.7fr)_1.3fr]">
      <form className={`${panel} grid content-start gap-3`} onSubmit={submit}>
        ...
        {tab==='employee'&&<><Input label="Ad" value={form.firstName} onChange={(v)=>change('firstName',v)} required/><Input label="Soyad" value={form.lastName} onChange={(v)=>change('lastName',v)} required/><CustomerLookupField value={form.customerId} onChange={(v)=>change('customerId',v)}/><Input label="Kullanıcı ID (opsiyonel)" type="number" value={form.userId} onChange={(v)=>change('userId',v)}/><Input label="QR kodu" value={form.qrCode} onChange={(v)=>change('qrCode',v)} required/><Input label="İşe giriş tarihi" type="date" value={form.employmentStartDate} onChange={(v)=>change('employmentStartDate',v)} required/></>}
...
function CustomerLookupField({value,onChange}:{value?:string;onChange:(value:string)=>void}) { return <label className="grid gap-1 text-xs font-bold uppercase">Entegre cari<PagedAppDropdown queryKey="kkd-customer-lookup" fetchPage={kkdApi.customersPaged} toOption={(item)=>({value:String(item.id),label:`${item.code} · ${item.name}`})} value={value||null} onValueChange={onChange} placeholder="Cari kodu veya adıyla seçin" searchPlaceholder="Cari ara" searchable minSearchLength={1} searchFields={['code','name']} /></label>; }

function MatrixFields({form,change}:{form:Record<string,string>;change:(k:string,v:string)=>void}) { return <>
  <CustomerLookupField value={form.customerId} onChange={(v)=>change('customerId',v)}/>
  <label className="grid gap-1 text-xs font-bold uppercase">Stok grubu<PagedAppDropdown queryKey="kkd-stock-group-lookup" fetchPage={kkdApi.stockGroupsPaged} toOption={(item)=>({value:item.code,label:item.code,description:`${item.stockCount} stok`})} value={form.groupCode||null} onValueChange={(value)=>{change('groupCode',value);change('stockId','');change('stockLookupValue','');}} placeholder="Stok grubu seçin" searchPlaceholder="Grup kodu ara" searchable minSearchLength={1} searchFields={['code']} /></label>
  <label className="grid gap-1 text-xs font-bold uppercase">Stok <span className="font-medium normal-case text-slate-500">(boş bırakılırsa grup kuralı)</span><PagedAppDropdown queryKey={['kkd-stock-lookup',form.groupCode||'all']} fetchPage={(request)=>kkdApi.stocksPaged(request,form.groupCode)} toOption={(item)=>({value:`${item.id}|${encodeURIComponent(item.groupCode||'')}`,label:`${item.code} · ${item.name}`,description:[item.groupCode,item.unitCode].filter(Boolean).join(' · ')})} value={form.stockLookupValue||null} onValueChange={(value)=>{if(!value){change('stockId','');change('stockLookupValue','');return;}const [id,group='']=value.split('|');change('stockId',id);change('stockLookupValue',value);if(group)change('groupCode',decodeURIComponent(group));}} staticOptions={[{value:'',label:'Tüm stok grubu için uygula'}]} placeholder="İsteğe bağlı stok seçin" searchPlaceholder="Stok kodu veya adıyla ara" searchable minSearchLength={1} searchFields={['code','name']} /></label>
  <Input label="Standart kodu" value={form.standardCode} onChange={(v)=>change('standardCode',v)}/>
...
export function KkdEntitlementPage() {
  ...
  return <Page title="KKD Hak Sorgulama" description="Stok özel kuralı, grup kuralı, faz, sıklık, yıllık sınır ve ek hak birlikte hesaplanır."><form className={`${panel} grid gap-3 md:grid-cols-4`} onSubmit={(e)=>{e.preventDefault();mutation.mutate()}}><label className="grid gap-1 text-xs font-bold uppercase">Personel<select className={field} value={employeeId} onChange={(e)=>setEmployeeId(e.target.value)} required><option value="">Seçin</option>{employees.data?.map((x)=><option key={x.id} value={x.id}>{x.employeeCode} · {x.fullName}</option>)}</select></label><label className="grid gap-1 text-xs font-bold uppercase">Stok<PagedAppDropdown queryKey="kkd-entitlement-stock-lookup" fetchPage={(request)=>kkdApi.stocksPaged(request)} toOption={(item)=>({value:String(item.id),label:`${item.code} · ${item.name}`,description:[item.groupCode,item.unitCode].filter(Boolean).join(' · ')})} value={stockId||null} onValueChange={setStockId} placeholder="Stok kodu veya adıyla seçin" searchPlaceholder="Stok ara" searchable minSearchLength={1} searchFields={['code','name']} /></label><Input label="Miktar" type="number" value={quantity} onChange={setQuantity} required/><button className="self-end min-h-11 rounded-xl bg-cyan-500 font-black text-slate-950">Kontrol et</button></form>{result&&<div className={`${panel} border-l-4 ${result.isAllowed?'border-l-emerald-500':'border-l-rose-500'}`}><div className="flex items-center gap-3">{result.isAllowed?<CheckCircle2 className="text-emerald-500"/>:<ShieldCheck className="text-rose-500"/>}<div><h2 className="font-black">{result.isAllowed?'Teslime uygun':'Teslime uygun değil'}</h2><p className="text-sm text-slate-500">{result.message}</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-4"><Metric label="Grup" value={result.groupCode}/><Metric label="Faz" value={result.phaseType||'-'}/><Metric label="Ana hak" value={result.matrixRemainingQuantity}/><Metric label="Ek hak" value={result.overrideRemainingQuantity}/></div></div>}</Page>;
}
...
export function KkdDistributionsPage() {
  ...
  const decision=useMutation({mutationFn:({id,approve}:{id:number;approve:boolean})=>kkdApi.decideExcessApproval(id,approve,reasons[id]||''),onSuccess:async()=>{toast.success('Kota aşım kararı kaydedildi.');await qc.invalidateQueries({queryKey:['kkd','distributions']});},onError:e=>toast.error(message(e))});
  return <Page title="KKD Dağıtımları" description="Teslim kaydı, hak tüketimi, fiziksel ambar çıkışı ve ERP sonucu aynı belge zincirinde izlenir."><div className={`${panel} overflow-auto`}><table className="w-full min-w-[1180px] text-left text-sm"><thead><tr className="border-b border-[var(--wms-app-border)]">{['Belge','Personel','Toplam','Hak','Fazla','Kota aşım onayı','Durum','Ambar çıkışı'].map(x=><th className="p-3" key={x}>{x}</th>)}</tr></thead><tbody>{query.data?.map(x=><tr className="border-b border-[var(--wms-app-border)]" key={x.id}><td className="p-3 font-bold">{x.documentNo}</td><td className="p-3">{x.employeeCode} · {x.employeeName}</td><td className="p-3">{x.totalQuantity}</td><td className="p-3">{x.entitledQuantity}</td><td className="p-3">{x.excessQuantity}</td><td className="p-3"><strong className={x.excessApprovalStatus==='Pending'?'text-amber-500':x.excessApprovalStatus==='Approved'?'text-emerald-500':x.excessApprovalStatus==='Rejected'?'text-rose-500':''}>{x.excessApprovalStatus}</strong>{x.excessApprovalReason&&<div className="mt-1 text-xs text-slate-500">{x.excessApprovalReason}</div>}{can('WMS.KKD.OVERRIDES.MANAGE')&&x.excessApprovalStatus==='Pending'&&<div className="mt-2 flex min-w-[300px] gap-2"><input className={`${field} min-w-44`} value={reasons[x.id]||''} onChange={e=>setReasons(v=>({...v,[x.id]:e.target.value}))} placeholder="Fiziksel kontrol notu"/><button disabled={decision.isPending||(reasons[x.id]||'').trim().length<5} onClick={()=>decision.mutate({id:x.id,approve:true})} className="rounded-lg bg-emerald-600 px-3 font-bold text-white disabled:opacity-50">Onayla</button><button disabled={decision.isPending||(reasons[x.id]||'').trim().length<5} onClick={()=>decision.mutate({id:x.id,approve:false})} className="rounded-lg bg-rose-600 px-3 font-bold text-white disabled:opacity-50">Reddet</button></div>}</td><td className="p-3">{x.status}</td><td className="p-3">{x.warehouseOutboundId?<Link className="font-bold text-cyan-500" to={`/warehouse/warehouse-outbounds/${x.warehouseOutboundId}/operations`}>Operasyonu aç</Link>:'-'}</td></tr>)}</tbody></table></div></Page>;
}
