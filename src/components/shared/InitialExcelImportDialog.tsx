...
export function InitialExcelImportDialog<T>({
  ...
  const download = async () => {
    ...
    try {
      ...
      toast.success('Güncel sistem verileriyle Excel şablonu indirildi.');
    } catch (error) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      setResult(imported);
      await onImported();
      toast.success('Excel aktarımı başarıyla tamamlandı.');
    } catch (error) {
      ...
      <OpsDialogContent size="xl">
        ...
        <OpsDialogBody className="space-y-5">
          <div className="flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-200">
            ...
            <div><strong className="block">İlk aktarım güvenlik kuralı</strong><p>{warning}</p></div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <section className="rounded-xl border p-4">
              <h3 className="font-semibold">1. Güncel şablonu indirin</h3>
              <p className="mt-1 text-sm text-slate-500">Depo, raf, stok ve diğer referanslar indirme anındaki sistem verilerinden hazırlanır.</p>
              <button type="button" disabled={Boolean(busy)} onClick={() => void download()} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-[var(--wms-brand-primary)] px-4 py-2 text-sm font-semibold text-[var(--wms-brand-primary)] disabled:opacity-50">
                {busy === 'download' ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}Şablonu indir
              </button>
            </section>
